(function ($) {
    $.labTestResultCarriedOuts = function (options) {
        // Default options
        var settings = $.extend({
            baseUrl: "/",
            formSelector: "#labTestResultCarriedOut-form",
            formContainer: ".js-labTestResultCarriedOut-form-container",
            gridSelector: "#labTestResultCarriedOut-grid",
            gridContainer: ".js-labTestResultCarriedOut-grid-container",
            editSelector: ".js-labTestResultCarriedOut-edit",
            saveSelector: ".js-labTestResultCarriedOut-save",
            selectAllSelector: "#labTestResultCarriedOut-check-all",
            deleteSelector: ".js-labTestResultCarriedOut-delete-confirm",
            deleteModal: "#labTestResultCarriedOut-delete-modal",
            finalDeleteSelector: ".js-labTestResultCarriedOut-delete",
            clearSelector: ".js-labTestResultCarriedOut-clear",
            topSelector: ".js-go",
            decimalSelector: ".js-labTestResultCarriedOut-decimalplaces",
            maxDecimalPlace: 5,
            showNagativeFormat: false,
            availabilitySelector: ".js-labTestResultCarriedOut-check-availability",
            haseFile: false,
        ...
        $(() => {
            ...
            loadLabTestResultCarriedOuts(settings.baseUrl, settings.gridSelector);
            initialize();
            ...
            $("body").on("keyup", settings.availabilitySelector, function () {
                var self = $(this);
                let code = $(".js-labTestResultCarriedOut-code").val();
                let name = self.val();
            ...
            $("body").on("click", ".js-export", function () {
                ...
                window.open(
                    settings.baseUrl + `/Export?categoryCode=${categoryCode}&reportType=LabLabTestResultCarriedOuts&reportRenderType=${reportRenderType}`,
                    "_blank"
                )
            });
        });
        ...
        function generateCode() {
            $.ajax({
                ...
                success: function (response) {
                    if (response.isSuccess)
                        $("#CarriedOutById").val(response.message);
                    else
                        toastr.error(response.message);
                }
            });
        }
        function loadLabTestResultCarriedOuts(baseUrl, gridSelector) {
            var data = {
            ...
            var dataTable = $(gridSelector).DataTable({
                ...
                columns: [
                    {
                        "data": "carriedOutById", "className": "text-center", "render": function (data) {
                            return ` <input type="checkbox" class="checkBox" value="${data}" />`;
                        }
                    },
                    {
                        "data": "carriedOutById", "render": function (data) {
                            return `<a class='btn js-labTestResultCarriedOut-edit' data-id='${data}'><i>${data}</i></a>`;
                        }
                    },
                    { "data": "carriedOutBy", "autowidth": true, "className": "text-center" },
