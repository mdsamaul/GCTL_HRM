(function ($) {
    $.commissionPaymentReports = function (options) {
        // Default options
        var settings = $.extend({
            baseUrl: "/",
            formSelector: "#commission-payment-form",
            formContainer: ".js-commission-payment-form-container",
            gridSelector: "#commission-payments-grid",
            gridContainer: ".js-commission-payment-grid-container",
            previewSelector: ".js-commission-payment-preview",
            clearSelector: ".js-commission-payment-clear",
            topSelector: ".js-go",
            load: function () {
        ...
        $(() => {
            ...
            $("body").on("click", ".js-commission-payment-export", function () {
                var self = $(this);
                ...
                window.open(
                    settings.baseUrl + `/Export?commissionReceiver=${commissionReceiver}&commissionReceiverId=${commissionReceiverId}&fromDate=${fromDate}&toDate=${toDate}&reportType=CommissionPayment&reportRenderType=${reportRenderType}`,
                    "_blank"
                )
            });
        ...
        function previewReport() {
            $(".js-commission-payment-grid-container").LoadingOverlay("show", {
            ...
            $.ajax({
                ...
                data: {
                    ...
                    toDate: $("#ToDate").val() ?? "",
                    reportType: "CommissionPayment",
                    reportRenderType: reportRenderType,
                    isPreview: true
                },
                success: function (response) {
                    var url = normalizeUrl(getBaseUrl()) + response;
                    $("#js-commission-payment-previewer").attr("data", url);
                    $(".js-commission-payment-grid-container").LoadingOverlay("hide", true);
                }
            });
        }
