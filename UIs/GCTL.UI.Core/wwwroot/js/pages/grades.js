(function ($) {
    $.grades = function (options) {
        // Default options
        var settings = $.extend({
            baseUrl: "/",
            formSelector: "#Grades-form",
            formContainer: ".js-Grades-form-container",
            gridSelector: "#Grades-grid",
            gridContainer: ".js-Grades-grid-container",
            editSelector: ".js-Grades-edit",
            saveSelector: ".js-Grades-save",
            selectAllSelector: "#Grades-check-all",
            deleteSelector: ".js-Grades-delete-confirm",
            deleteModal: "#Grades-delete-modal",
            finalDeleteSelector: ".js-Grades-delete",
            clearSelector: ".js-Grades-clear",
            topSelector: ".js-go",
            decimalSelector: ".js-Grades-decimalplaces",
            maxDecimalPlace: 5,
            showNagativeFormat: false,
            availabilitySelector: ".js-Grades-check-availability",
            haseFile: false,
        ...
        $(() => {
            ...
            $("body").on("keyup", settings.availabilitySelector, function () {
                var self = $(this);
                let code = $(".js-GradeCode-code").val();
                let name = self.val();
        ...
        function validation() {
            ...
            var degName = $('#GradeName').val();
            var gradeType = $('#GradeTypeId').val();
            if (!degName) {
                toastr.info('Enter Grade Name');
                $('#GradeName').trigger('focus');
                return false;
            }


            if (!gradeType) {
                toastr.info('Select Grade Type');
                $('#GradeTypeId').trigger('open');
                return false;
            }
        ...
        //
        function initialize() {
            $(settings.formSelector + ' .select2HrmGradeType').select2({
                width: '100%',
                language: {
                    noResults: function () {
