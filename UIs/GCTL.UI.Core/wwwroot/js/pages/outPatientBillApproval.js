(function ($) {
    $.outPatientBillApproval = function (options) {
        // Default options
        var settings = $.extend({
            baseUrl: "/",
            formSelector: "#doctor-form",
            formContainer: ".js-doctor-form-container",
            gridSelector: "#outPatientBillApproval-grid",
            gridContainer: ".js-outPatientBillApproval-grid-container",
        ...
        $(() => {
            ...
            loadOutPatientBill(settings.baseUrl, settings.gridSelector);
            $("body").on("click", '.outPatientBillApproval-check-all', function () {
                if ($(this).is(":checked")) {
            ...
            $("body").on("click", '.js-outPatientBillApproval-preview', function () {
                loadOutPatientBill(settings.baseUrl, settings.gridSelector);
            });


            $("body").on("click", ".js-outPatientBillApprovalPrintPreview", function () {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        window.open(
                            normalizeUrl(getBaseUrl()) + `/Preview/OutPatientBill`,
                            "_blank"
                        )
                    }
                });
            ...
            $("body").on("click", ".js-Approved", function () {
                event.preventDefault();
                var billEntryNo = "";
                $("#outPatientBillApproval-grid TBODY TR").each(function () {
                    var row = $(this);
        ...
        function loadOutPatientBill(baseUrl, gridSelector) {
            var data = {
                    ...
                    { "data": "isApproved", "className": "text-center", width: "130px" },
                    {
                        "data": "billEntryNo", "render": function (data, type, row) {
                            return `<div class='action-buttons p-1'>
                                           <button type="button" class="btn btn-danger btn-circle btn-sm js-outPatientBillApprovalPrintPreview"
                                                data-id="${data}">
