...
{
    public class AttendanceMovementRegisterReportController : BaseController
    {
        private readonly IAttendanceMovementRegisterReportService attendanceService;
        private readonly IRepository<HrmPayMonth> monthRepo;

        public AttendanceMovementRegisterReportController(
            IAttendanceMovementRegisterReportService attendanceService,
        ...
        private void InjectLoginInfo(AttendanceMovementRegisterReportFilterData req)
        {
        ...
        {
            var hasPermission = await attendanceService.PagePermissionAsync(LoginInfo.AccessCode);
            if (!hasPermission)
            {
            ...
            AttendanceMovementRegisterReportViewModel model = new AttendanceMovementRegisterReportViewModel()
            {
        ...
        [HttpPost]
        public async Task<IActionResult> GetAttendanceMachineData([FromBody] AttendanceMovementRegisterReportFilterData filter)
        {
            try
            {
                InjectLoginInfo(filter);
                var result = await attendanceService.GetAttendanceMachineDataAsync(filter);
                if (result != null)
                {
        ...
        [HttpPost]
        public async Task<IActionResult> GetFilters([FromBody] AttendanceMovementRegisterReportFilterData filter)
        {
            try
            {
                var result = await attendanceService.GetAttendanceMachineDataFiltersAsync(filter);
                if (result != null)
                {
                    return Json(new { isSuccess = true, message = "successed data load", data = result });
                }
        ...
        [HttpPost]
        public async Task<IActionResult> ExcelDownload([FromBody] AttendanceMovementRegisterReportFilterData filter)
        {
            try
            {
                var result = await attendanceService.GetAttendanceMachineDataAsync(filter);
        ...
        public IActionResult ExportAttendanceMovementRegisterExcelAsync(List<DepartmentGroupedData> groupedData, AttendanceMovementRegisterReportFilterData filter)
        {
            try
            {
